...
// ── type metadata (label shown in group header, pill text, accent color) ──────
const TYPE_META = {
...
// ── which palette types map to a DetailModal type string ─────────────────────
// DetailModal supports: "customer" | "delivery" | "expense" | "supply" | "wastage"
// Everything else → navigate to tab
...
function buildIndex({ customers, deliveries, expenses, supplies, wastage, products, staffList, machineList, vehList, ingItems }) {
  ...
  NAV_TABS.forEach(tab => items.push({
    _type: "nav", _id: tab.id,
    icon: tab.icon, label: tab.id, sub: "Go to tab", keywords: tab.id,
  }));

  QUICK_ACTIONS.forEach(a => items.push({
    _type: "action", _id: a.id,
    icon: a.icon, label: a.label, sub: "Quick action", tab: a.tab, keywords: a.label,
  }));

  customers.forEach(c => items.push({
    _type: "customer", _id: c.id, icon: "👤",
    label: c.name || c.id,
    sub: [c.phone, c.area || c.address].filter(Boolean).join(" · ") || "Customer",
    raw: c,
    keywords: [c.name, c.phone, c.area, c.address, c.id, c.notes].filter(Boolean).join(" "),
  }));
  ...
  [...deliveries]
    ...
    .forEach(d => items.push({
      _type: "delivery", _id: d.id,
      icon: d.status === "Delivered" ? "✅" : d.status === "Cancelled" ? "❌" : "🚚",
      label: d.customer || d.id,
      sub: [d.date, d.status, d.invoiceNo || d.invNo].filter(Boolean).join(" · "),
      raw: d,
      keywords: [d.customer, d.invoiceNo, d.invNo, d.id, d.date, d.status, d.agent, d.createdBy, d.notes, d.address].filter(Boolean).join(" "),
    }));

  expenses.forEach(e => items.push({
    _type: "expense", _id: e.id, icon: "💸",
    label: e.category || "Expense",
    sub: [e.date, e.vendor, inr(e.amount || 0)].filter(Boolean).join(" · "),
    raw: e,
    keywords: [e.category, e.vendor, e.date, e.notes, e.tags, e.paymentMethod, e.approvedBy, String(e.amount || "")].filter(Boolean).join(" "),
  }));

  supplies.forEach(s => items.push({
    _type: "supply", _id: s.id, icon: "📦",
    label: s.item || s.id,
    sub: [s.date, s.supplier, s.qty != null ? `${s.qty} ${s.unit || ""}`.trim() : ""].filter(Boolean).join(" · "),
    raw: s,
    keywords: [s.item, s.supplier, s.date, s.notes, String(s.qty || ""), s.unit].filter(Boolean).join(" "),
  }));

  wastage.forEach(w => items.push({
    _type: "wastage", _id: w.id, icon: "♻️",
    label: w.product || "Wastage",
    sub: [w.date, w.type, w.qty != null ? `${w.qty} ${w.unit || ""}`.trim() : ""].filter(Boolean).join(" · "),
    raw: w,
    keywords: [w.product, w.type, w.date, w.reason, w.shift, w.loggedBy, String(w.qty || "")].filter(Boolean).join(" "),
  }));

  products.forEach(p => items.push({
    _type: "product", _id: p.id, icon: "🥙",
    label: p.name || p.id,
    sub: p.prices ? `₹${p.prices[0]}` : "Product",
    raw: p,
    keywords: [p.name, p.id, p.unit].filter(Boolean).join(" "),
  }));

  staffList.forEach(s => items.push({
    _type: "staff", _id: s.id, icon: "👷",
    label: s.name || s.id,
    sub: [s.role, s.phone, s.shift].filter(Boolean).join(" · "),
    raw: s,
    keywords: [s.name, s.role, s.phone, s.shift, s.department].filter(Boolean).join(" "),
  }));

  machineList.forEach(m => items.push({
    _type: "machine", _id: m.id, icon: "⚙️",
    label: m.name || m.id,
    sub: [m.type, m.location, m.status].filter(Boolean).join(" · "),
    raw: m,
    keywords: [m.name, m.type, m.location, m.status, m.serial].filter(Boolean).join(" "),
  }));

  vehList.forEach(v => items.push({
    _type: "vehicle", _id: v.id, icon: "🚛",
    label: v.name || v.regNo || v.id,
    sub: [v.type, v.regNo, v.driver].filter(Boolean).join(" · "),
    raw: v,
    keywords: [v.name, v.regNo, v.type, v.driver, v.status].filter(Boolean).join(" "),
  }));

  ingItems.forEach(i => items.push({
    _type: "ingredient", _id: i.id, icon: "🌾",
    label: i.name || i.id,
    sub: [i.unit, i.category, i.stock != null ? `Stock: ${i.stock}` : ""].filter(Boolean).join(" · "),
    raw: i,
    keywords: [i.name, i.unit, i.category, i.supplier].filter(Boolean).join(" "),
  }));
...
export function CommandPalette({
  ...
  const activate = useCallback(item => {
    ...
    if (modalType && item.raw) {
      onOpenDetail?.({ type: modalType, data: item.raw });
    } else {
      onNavigate?.(TAB_FALLBACK_MAP[item._type] || "Dashboard");
    }
  }, [onNavigate, onOpenDetail, onQuickAction]);

  if (!open) return (
    <>
      {/* Mobile/Tablet trigger button — fixed bottom-right, above BottomNav */}
      <button
        ...
        style={{
          ...
          width: 46, height: 46,
          borderRadius: "50%",
          background: dm ? "#1e2d45" : "#ffffff",
          border: `1.5px solid ${dm ? "#2a3d5a" : "#e2e8f0"}`,
          boxShadow: dm ? "0 4px 20px rgba(0,0,0,0.5)" : "0 4px 20px rgba(0,0,0,0.12)",
  ...
  const panel = {
    ...
    boxShadow: dm ? "0 -20px 60px rgba(0,0,0,0.6)" : "0 -10px 40px rgba(0,0,0,0.1)",
    overflow: "hidden",
    animation: isMobile ? "cp_up 0.22s cubic-bezier(0.16,1,0.3,1)" : "cp_in 0.14s cubic-bezier(0.16,1,0.3,1)",
    paddingBottom: isMobile ? "env(safe-area-inset-bottom, 0px)" : 0,
  };
  const kbdStyle = {
    fontSize: 10, color: dm ? "#4a6080" : "#94a3b8",
    background: dm ? "#1e2736" : "#f1f5f9",
    border: `1px solid ${dm ? "#2a3347" : "#e2e8f0"}`,
    borderRadius: 5, padding: "2px 6px", fontFamily: "inherit", fontWeight: 700,
  };
  ...
  return (
    <>
      ...
      <div style={overlay} onMouseDown={() => setOpen(false)}>
        <div style={panel} onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"14px 18px", borderBottom:`1px solid ${dm?"#1e2736":"#f0f4f8"}` }}>
            ...
            <input
              ...
              placeholder="Search customers, deliveries, expenses, supplies…"
              value={query}
          ...
          {/* results */}
          <div ref={listRef} style={{ maxHeight: isMobile ? "55vh" : 400, overflowY:"auto", padding:"4px 0 8px", WebkitOverflowScrolling:"touch" }} className="cp-list">
            {results.length === 0 ? (
              ...
              return (
                <React.Fragment key={item._id + i}>
                  ...
                  >
                    ...
                    <span style={{ fontSize:13, fontWeight:600, flex:1, minWidth:0, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", color:active?(dm?"#93c5fd":"#1d4ed8"):(dm?"#e8edf5":"#0f172a") }}>
                      {item.label}
                    </span>
                    {item.sub && (
                      <span style={{ fontSize:11, color:dm?"#3d5266":"#94a3b8", flexShrink:0, maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {item.sub}
                      </span>
                    )}

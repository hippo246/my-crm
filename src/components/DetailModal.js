 ...
 * DetailModal.js
 * ─────────────────────────────────────────────────────────────────────────────
 * Reads ALL data from useCRM() — no more prop drilling.
 * CRM.js just renders: <DetailModal modal={detailModal} onClose={closeDetail} />
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
function DetailModalInner({ modal, onClose }) {
  ...
  const panelStyle = {
    ...
    width: "100%", maxWidth: 560, height: "auto", maxHeight: "min(92dvh,92svh,92vh)",
  ...
  const scrollStyle = {
    ...
    padding: "0 22px 20px", overscrollBehavior: "contain",
  };
  ...
  const Row = ({ label, val, color, onClick }) => (
    <div onClick={onClick} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${t.border}`, cursor: onClick ? "pointer" : "default" }}
      onMouseEnter={ev => { if (onClick) ev.currentTarget.style.background = t.inp + "88"; }}
  ...
  if (type === "expense") {
    ...
    const vendorExps = e.vendor
      ? [...expenses.filter(x => x.vendor === e.vendor)].sort((a, b) => (b.date || "").localeCompare(a.date || ""))
      : [];
      ...
      <div style={overlayStyle} onClick={ev => { if (ev.target === ev.currentTarget) onClose(); }}>
        <div style={panelStyle}>
          ...
          <div style={scrollStyle}>
            ...
            {e.notes && (
              <div style={{ background: t.inp, borderRadius: 12, padding: "12px 14px", margin: "12px 0" }}>
              ...
              <div style={{ margin: "4px 0 12px" }}>
                ...
                <div style={{ border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
                  {vendorExps.map((ve, vi) => (
                    <div key={ve.id || vi}
                      style={{ padding: "8px 12px", display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, borderTop: vi > 0 ? `1px solid ${t.border}` : "none", background: ve.id === e.id ? (t.inp + "aa") : "transparent", cursor: "pointer" }}
            ...
            {isAdmin && (
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={() => { setEf({ ...e, amount: String(e.amount) }); setEsh(e); onClose(); }}
                  style={{ flex: 1, padding: "12px", borderRadius: 12, background: t.inp, border: `1.5px solid ${t.border}`, color: t.text, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>✏️ Edit</button>
                <button onClick={() => { delE(e); onClose(); }}
                  style={{ flex: 1, padding: "12px", borderRadius: 12, background: "#dc2626", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", border: "none" }}>🗑️ Delete</button>
              </div>
            )}
            ...
            <ActivitySection
              ...
              actLog={actLog}
              entityId={e.id}
              entityLabel={`${e.category}${e.vendor ? ` ${e.vendor}` : ""}`}
              entityType="expense"
              t={t}
            />
          </div>
      ...
      <div style={overlayStyle} onClick={ev => { if (ev.target === ev.currentTarget) onClose(); }}>
        <div style={panelStyle}>
          ...
          <div style={scrollStyle}>
              ...
              <div style={{ margin: "12px 0" }}>
                ...
                {cProducts.map(p => (
                  <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${t.border}` }}>
                    <div>
                      <p style={{ color: t.text, fontSize: 12, fontWeight: 600 }}>{p.name}</p>
                      <p style={{ color: t.sub, fontSize: 10 }}>{p.qty} units total</p>
                    </div>
                ...
                <div style={{ border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ maxHeight: 300, overflowY: "auto" }}>
                    {[...cDelivs].sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((d, di) => {
                      ...
                      return (
                        <div key={d.id || di}
                          style={{ padding: "10px 12px", borderTop: di > 0 ? `1px solid ${t.border}` : "none", cursor: "pointer", transition: "background .12s" }}
                            ...
                            <div>
                              ...
                              {d.orderLines && (
                                <p style={{ color: t.sub, fontSize: 10, marginTop: 2 }}>
                                  {Object.entries(d.orderLines).filter(([, l]) => l.qty > 0).map(([pid, l]) => {
                                    const prod = products.find(p => p.id === pid);
                                    return `${l.qty}×${prod?.name || l.name || pid}`;
                                  }).join(", ")}
                                </p>
                              )}
            ...
            {isAdmin && cDue > 0 && (
              <button onClick={() => { setPaySh(c); setPayAmt(""); onClose(); }}
                style={{ width: "100%", padding: "13px", borderRadius: 12, background: "#10b981", color: "#fff", fontWeight: 800, fontSize: 14, cursor: "pointer", border: "none", marginTop: 8 }}>
                💰 Record Payment — {inr(cDue)} due
              </button>
            )}
      ...
      <div style={overlayStyle} onClick={ev => { if (ev.target === ev.currentTarget) onClose(); }}>
        <div style={panelStyle}>
          ...
          <div style={scrollStyle}>
            ...
            {/* Replacement */}
            {d.replacement?.done && (
              <div style={{ background: "#f9731615", border: "1px solid #f9731630", borderRadius: 12, padding: "12px 14px", margin: "8px 0" }}>
            ...
            {/* Partial payment */}
            {d.partialPayment?.enabled && (+d.partialPayment?.amount || 0) > 0 && (
              <div style={{ background: "#10b98115", border: "1px solid #10b98130", borderRadius: 12, padding: "12px 14px", margin: "8px 0" }}>
            ...
            {/* Customer quick-link */}
            {cust.id && (
              <button onClick={() => setDetailModal({ type: "customer", data: cust })}
                style={{ width: "100%", padding: "12px", borderRadius: 12, background: t.inp, border: `1.5px solid ${t.border}`, color: t.text, fontWeight: 700, fontSize: 13, cursor: "pointer", marginTop: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={scrollStyle}>
            ...
            {dayDelivs.length > 0 && (
              <div style={{ margin: "8px 0 16px" }}>
                ...
                {dayDelivs.map((d, di) => {
                  ...
                  return (
                    <div key={d.id || di}
                      style={{ padding: "10px 12px", background: t.inp, borderRadius: 10, marginBottom: 6, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                      onClick={() => setDetailModal({ type: "delivery", data: d })}>
                      <div>
            ...
            {dayExp.length > 0 && (
              <div style={{ margin: "8px 0 16px" }}>
                ...
                {dayExp.map((e, ei) => (
                  <div key={e.id || ei}
                    style={{ padding: "10px 12px", background: "#ef444412", border: "1px solid #ef444430", borderRadius: 10, marginBottom: 6, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
            ...
            {daySup.length > 0 && (
              <div style={{ margin: "8px 0 16px" }}>
                ...
                {daySup.map((s, si) => (
                  <div key={s.id || si} style={{ padding: "10px 12px", background: "#8b5cf612", border: "1px solid #8b5cf630", borderRadius: 10, marginBottom: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <p style={{ color: t.text, fontSize: 12, fontWeight: 600 }}>{s.item}</p>
                      {s.supplier && <p style={{ color: t.sub, fontSize: 10 }}>{s.supplier}</p>}
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <p style={{ color: "#8b5cf6", fontWeight: 800, fontSize: 12 }}>{inr(s.cost || 0)}</p>
                      <p style={{ color: t.sub, fontSize: 10 }}>{s.qty} {s.unit}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {dayWaste.length > 0 && (
              <div style={{ margin: "8px 0 16px" }}>
                ...
                {dayWaste.map((w, wi) => (
                  <div key={w.id || wi} style={{ padding: "10px 12px", background: "#f9731612", border: "1px solid #f9731630", borderRadius: 10, marginBottom: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <p style={{ color: t.text, fontSize: 12, fontWeight: 600 }}>{w.product}</p>
                      <p style={{ color: t.sub, fontSize: 10 }}>{w.qty} {w.unit} · {w.type}</p>
                    </div>
  ...
  if (type === "agent") {
    ...
    const agentDelivs = [...deliveries.filter(d => d.createdBy === agentName || d.agent === agentName)]
    ...
    const agentExps = [...expenses.filter(e => e.approvedBy === agentName)]
      .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
          ...
          <div style={scrollStyle}>
                ...
                <div style={{ border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ maxHeight: 320, overflowY: "auto" }}>
                    {agentDelivs.map((d, di) => {
                      ...
                      return (
                        <div key={d.id || di}
                          style={{ padding: "10px 12px", borderTop: di > 0 ? `1px solid ${t.border}` : "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                          onClick={() => setDetailModal({ type: "delivery", data: d })}
                          onMouseEnter={ev => { ev.currentTarget.style.background = t.inp + "88"; }}
                          onMouseLeave={ev => { ev.currentTarget.style.background = "transparent"; }}>
                          <div>
            ...
            {agentExps.length > 0 && (
              <div style={{ margin: "8px 0 16px" }}>
                ...
                {agentExps.map((e, ei) => (
                  <div key={e.id || ei}
                    style={{ padding: "9px 12px", background: "#ef444412", borderRadius: 10, marginBottom: 6, display: "flex", justifyContent: "space-between", cursor: "pointer" }}
                    onClick={() => setDetailModal({ type: "expense", data: e })}>
                    <div>
                      <p style={{ color: t.text, fontSize: 12, fontWeight: 600 }}>{e.category}</p>
                      <p style={{ color: t.sub, fontSize: 10 }}>{e.date}</p>
                    </div>
                    <span style={{ color: "#ef4444", fontWeight: 800, fontSize: 12 }}>{inr(e.amount)}</span>
                  </div>
                ))}
            ...
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
              <div style={{ maxHeight: 400, overflowY: "auto" }}>
                {[...catExpenses].sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((e, ei) => (
                  <div key={e.id || ei}
                    style={{ padding: "10px 12px", borderTop: ei > 0 ? `1px solid ${t.border}` : "none", cursor: "pointer", transition: "background .12s" }}
                    onClick={() => setDetailModal({ type: "expense", data: e })}
                    onMouseEnter={ev => { ev.currentTarget.style.background = t.inp + "88"; }}
                    onMouseLeave={ev => { ev.currentTarget.style.background = "transparent"; }}>

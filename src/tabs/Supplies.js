...
export default function SuppliesTab({
  ...
  dm,
  // permissions
  isAdmin, can, canSeeFinancials,
  // data
  supplies, settings, srch,
  fSup,               // filtered supplies (memoized in CRM.js)
  totalSupC,          // memoized in CRM.js
  lowStockItems,      // memoized in CRM.js
  // actions
  ...
  return (<>
    ...
    {canSeeFinancials && <div className="crm-grid-4">
      ...
      {/* Low Stock */}
      <div style={{
        background: dm ? lowStockItems.length > 0 ? "linear-gradient(135deg,#2d0a0a,#3f1010)" : "linear-gradient(135deg,#0a1f0a,#0f2d0f)" : lowStockItems.length > 0 ? "linear-gradient(135deg,#fff1f2,#ffe4e6)" : "linear-gradient(135deg,#f0fdf4,#dcfce7)",
        border: `1px solid ${lowStockItems.length > 0 ? (dm ? "#7f1d1d" : "#fecdd3") : (dm ? "#14532d" : "#bbf7d0")}`,
    ...
    <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 16, padding: "12px 14px" }} className="flex items-center justify-between gap-3 flex-wrap">
      ...
      <div className="flex gap-3 items-center">
        {can("sup_export") && <div className="flex gap-2">
          <button onClick={() => exportTabPDF("Supplies", supplies, [{ label: "Item", key: "item" }, { label: "Qty", key: "qty", num: true }, { label: "Unit", key: "unit" }, { label: "Supplier", key: "supplier" }, { label: "Cost (₹)", key: "cost", num: true }, { label: "Date", key: "date" }, { label: "Notes", key: "notes" }], settings)}
            style={{ background: t.inp, color: t.sub, border: `1px solid ${t.border}`, borderRadius: 8, padding: "6px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>PDF</button>
          <button onClick={() => exportTabExcel("Supplies", supplies, [{ label: "Item", key: "item" }, { label: "Qty", key: "qty", num: true }, { label: "Unit", key: "unit" }, { label: "Supplier", key: "supplier" }, { label: "Cost", key: "cost", num: true }, { label: "Min Stock", key: "minStock" }, { label: "Date", key: "date" }, { label: "Notes", key: "notes" }], settings)}
            style={{ background: t.inp, color: t.sub, border: `1px solid ${t.border}`, borderRadius: 8, padding: "6px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>XLS</button>
        </div>}
        {can("sup_add") && <button onClick={() => { setSf(blkS()); setSsh("add"); }}
          style={{ background: "#2563eb", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer", WebkitTapHighlightColor: "transparent", touchAction: "manipulation", display: "flex", alignItems: "center", gap: 6, minHeight: 40, boxShadow: "0 2px 8px rgba(37,99,235,0.35)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
            <thead>
              <tr style={{ background: dm ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.025)", borderBottom: `1.5px solid ${t.border}` }}>
                ...
                {["Item", "Supplier", "Date"].map(h => <th key={h} style={{ padding: "11px 12px", textAlign: "left", color: t.sub, fontSize: 10, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase", whiteSpace: "nowrap" }}>{h}</th>)}
              ...
              {sortedSup.map((s, si) => {
                ...
                return <tr key={s.id || si}
                  ...
                  <td style={{ padding: "14px 12px", verticalAlign: "middle" }}>
                    {s.minStock
                      ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: isLow ? "#ef444415" : "#10b98115", color: isLow ? "#dc2626" : "#059669", border: `1px solid ${isLow ? "#ef444425" : "#10b98125"}`, borderRadius: 99, padding: "3px 10px", fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>
                          {isLow ? "⚠️ Low" : "✓ OK"}
                        </span>

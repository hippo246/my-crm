...
export function VehiclesTab({
  dm, t, isAdmin, isFactory, settings, inr,
  vehLogs, vehList, setVehList,
  vehSearch, setVehSearch,
  vehSubTab, setVehSubTab,
  vehSh, setVehSh,
  vehF, setVehF,
  vehItemSh, setVehItemSh,
  vehItemF, setVehItemF,
  saveVehFixed, saveVehItem, delVeh,
  ask, notify, safeArr,
}) {
  const tStr = today();
  ...
  return <>
    ...
    <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:14,display:"flex",overflow:"hidden",marginBottom:4}}>
      {[["log","📋","Trip Log"],["fleet","🚐","Fleet"]].map(([k,icon,lbl])=>(
        <button key={k} onClick={()=>setVehSubTab(k)}
          style={{flex:1,padding:"11px 8px",fontSize:13,fontWeight:vehSubTab===k?700:500,background:vehSubTab===k?t.accent:"transparent",color:vehSubTab===k?t.accentFg:t.sub,border:"none",cursor:"pointer",transition:"all 0.15s",display:"flex",alignItems:"center",justifyContent:"center",gap:5,overflow:"hidden"}}>
    ...
    {vehSubTab==="log"&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <input value={vehSearch} onChange={e=>setVehSearch(e.target.value)} placeholder="Search vehicle, driver…"
      ...
      {fLogs.length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{fontSize:32,marginBottom:8}}>🚐</p><p style={{color:t.sub,fontSize:14,fontWeight:600}}>No vehicle logs yet</p></div>
        : fLogs.map(l=>{
    ...
    {vehSubTab==="fleet"&&<>
      ...
      {(vehList||[]).length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{color:t.sub,fontSize:14}}>No vehicles added yet</p></div>
        : (vehList||[]).map(v=>{

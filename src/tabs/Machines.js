...
export function MachinesTab({
  dm, t, isAdmin, isFactory, settings, displayName, inr,
  machineLogs, machineList, setMachineList,
  machSearch, setMachSearch,
  machSubTab, setMachSubTab,
  machSh, setMachSh,
  machF, setMachF,
  machItemSh, setMachItemSh,
  machItemF, setMachItemF,
  saveMach, saveMachItem, delMach,
  ask, notify, safeArr,
}) {
  const tStr = today();
  ...
  const overdueMachines = (machineList||[]).filter(m=>{
    const last=(machineLogs||[]).filter(l=>l.machineName===m.name).sort((a,b)=>(b.date||"").localeCompare(a.date||""))[0];
    return last?.nextDue&&last.nextDue<tStr;
  });
  ...
  return <>
    ...
    <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:14,display:"flex",overflow:"hidden",marginBottom:4}}>
      {[["log","📋","Maintenance Log"],["machines","⚙️","Machines"]].map(([k,icon,lbl])=>(
        <button key={k} onClick={()=>setMachSubTab(k)}
          style={{flex:1,padding:"11px 8px",fontSize:13,fontWeight:machSubTab===k?700:500,background:machSubTab===k?t.accent:"transparent",color:machSubTab===k?t.accentFg:t.sub,border:"none",cursor:"pointer",transition:"all 0.15s",display:"flex",alignItems:"center",justifyContent:"center",gap:5,overflow:"hidden"}}>
          <span style={{flexShrink:0}}>{icon}</span><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{lbl}</span>
        </button>
      ))}
    </div>
    ...
    {machSubTab==="log"&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <input value={machSearch} onChange={e=>setMachSearch(e.target.value)} placeholder="Search machine or issue…"
      ...
      {fLogs.length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{fontSize:32,marginBottom:8}}>⚙️</p><p style={{color:t.sub,fontSize:14,fontWeight:600}}>No maintenance logs yet</p></div>
        : fLogs.map(l=>{
    ...
    {machSubTab==="machines"&&<>
      ...
      {(machineList||[]).length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{color:t.sub,fontSize:14}}>No machines added yet</p></div>
        : (machineList||[]).map(m=>{
    ...
    <Sheet dm={dm} open={!!machSh} onClose={()=>setMachSh(null)} title={machSh==="add"?"⚙️ Log Maintenance Event":"✏️ Edit Log"}>
      ...
      {settings?.machineShowPartsReplaced!==false&&<Inp dm={dm} label="Parts Replaced" value={machF.partsReplaced} onChange={e=>setMachF(f=>({...f,partsReplaced:e.target.value}))} placeholder="e.g. Belt, Motor, Filter"/>}

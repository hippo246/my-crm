...
import { SectionHeader, TabStatCards } from "../components/ui";
...
export default function ExpensesTab({
  ...
  dm,
  // data
  expenses, wastage, supplies, deliveries, settings,
  // memoized totals from CRM.js
  totalExpOp, totalSupC, totalRev, netProfit,
  // filter state
  expDateFilter, setExpDateFilter,
  expCatFilter,  setExpCatFilter,
  expCustomFrom, setExpCustomFrom,
  expCustomTo,   setExpCustomTo,
  expSearch,     setExpSearch,
  // actions
  setEsh, setEf, blkE,
  setDetailModal,
}) {
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div key={cat} style={{background:over?(dm?"rgba(220,38,38,0.12)":"#fff1f2"):(dm?"rgba(245,158,11,0.12)":"#fffbeb"),border:`1px solid ${over?"#ef4444":"#f59e0b"}`,borderRadius:14,padding:"11px 16px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:6}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                <p style={{color:over?"#dc2626":"#d97706",fontWeight:700,fontSize:13}}>{over?`${cat} budget exceeded!`:`${cat} budget at ${pct}%`}</p>
                <p style={{color:over?"#dc2626":"#d97706",fontSize:11,opacity:0.8}}>{inr(spent)} spent of {inr(budget)} this month</p>
              </div>
            </div>
          ...
          <div style={{display:"flex",gap:16,marginTop:8,flexWrap:"wrap"}}>
            {[["#10b981","Profit",pctProfit],["#8b5cf6","Supply",pctSupply],["#ef4444","Expenses",pctExp],["#f97316","Waste",pctWaste]].map(([c,l,p])=>(
              <span key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:t.sub}}>
                <span style={{width:8,height:8,borderRadius:2,background:c,display:"inline-block"}}/>{l} <strong style={{color:c}}>{p}%</strong>
              </span>
            ))}
          </div>
        </div>
      ...
      <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
        <div style={{padding:"13px 16px",borderBottom:`1px solid ${t.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <p style={{color:t.sub,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em"}}>All entries</p>

...
export function IngredientsTab({
  dm, t, isAdmin, settings, displayName,
  ingLogs, ingItems, setIngItems,
  ingSearch, setIngSearch,
  ingDateFilter, setIngDateFilter,
  ingSh, setIngSh,
  ingF, setIngF,
  ingItemSh, setIngItemSh,
  ingItemF, setIngItemF,
  saveIng, saveIngItem, delIng,
  ask, notify, safeArr,
}) {
  const tStr = today();
  ...
  return <>
    ...
    <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:14,display:"flex",overflow:"hidden",marginBottom:4}}>
      {[["log","📋 Log"],["stock","📦 Stock"]].map(([k,lbl])=>(
        <button key={k} onClick={()=>setIngDateFilter(k==="stock"?"all":ingDateFilter)}
          style={{flex:1,padding:"11px 0",fontSize:13,
            fontWeight:((ingDateFilter==="stock"&&k==="stock")||(ingDateFilter!=="stock"&&k==="log"))?700:500,
            background:((ingDateFilter==="stock"&&k==="stock")||(ingDateFilter!=="stock"&&k==="log"))?t.accent:"transparent",
            color:((ingDateFilter==="stock"&&k==="stock")||(ingDateFilter!=="stock"&&k==="log"))?t.accentFg:t.sub,
            border:"none",cursor:"pointer",transition:"all 0.15s"}}>{lbl}</button>
      ))}
    </div>
    ...
    {ingDateFilter!=="stock"&&<>
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        ...
        <select value={ingDateFilter} onChange={e=>setIngDateFilter(e.target.value)}
      ...
      {fLogs.length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{fontSize:32,marginBottom:8}}>🧂</p><p style={{color:t.sub,fontSize:14,fontWeight:600}}>No consumption logs yet</p></div>
        : fLogs.map(l=>(
    ...
    {ingDateFilter==="stock"&&<>
      ...
      {Object.values(stockMap).length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{color:t.sub,fontSize:14}}>No ingredients yet</p></div>

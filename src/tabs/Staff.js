...
import { Btn, Inp, Sel, Sheet, SectionHeader } from "../components/ui";

export function StaffTab({
  dm, t, isAdmin, isFactory, settings, displayName,
  staffLogs, staffList, setStaffList,
  staffSearch, setStaffSearch,
  staffDateFilter, setStaffDateFilter,
  staffSubTab, setStaffSubTab,
  staffSh, setStaffSh,
  staffF, setStaffF,
  staffMemberSh, setStaffMemberSh,
  staffMemberF, setStaffMemberF,
  saveStaff, saveStaffMember, delStaff,
  ask, notify, safeArr,
}) {
  ...
  return <>
    ...
    <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:14,display:"flex",overflow:"hidden",marginBottom:4}}>
      {[
        ["log","📋","Attendance Log"],
        ["roster","👥","Staff Roster"],
        ...(isAdmin ? [["management","🗂️","Management"],["reports","📊","Reports"],["settings","⚙️","Settings"]] : []),
      ].map(([k,icon,lbl])=>(
        <button key={k} onClick={()=>setStaffSubTab(k)}
          style={{flex:1,padding:"11px 8px",fontSize:13,fontWeight:staffSubTab===k?700:500,
            background:staffSubTab===k?t.accent:"transparent",color:staffSubTab===k?t.accentFg:t.sub,
            border:"none",cursor:"pointer",transition:"all 0.15s",display:"flex",alignItems:"center",justifyContent:"center",gap:5,overflow:"hidden"}}>
    ...
    {staffSubTab==="log"&&<>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <input value={staffSearch} onChange={e=>setStaffSearch(e.target.value)} placeholder="Search staff…"
          style={{flex:1,minWidth:160,background:t.inp,border:`1.5px solid ${t.inpB}`,color:t.text,borderRadius:10,padding:"9px 12px",fontSize:14,outline:"none"}}/>
        <select value={staffDateFilter} onChange={e=>setStaffDateFilter(e.target.value)}
          style={{background:t.inp,border:`1.5px solid ${t.inpB}`,color:t.text,borderRadius:10,padding:"9px 12px",fontSize:13,outline:"none",WebkitAppearance:"none"}}>
          {[["all","All time"],["today","Today"],["week","This week"]].map(([v,l])=><option key={v} value={v}>{l}</option>)}
        </select>
      </div>
      {fLogs.length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{fontSize:32,marginBottom:8}}>🧑‍🍳</p><p style={{color:t.sub,fontSize:14,fontWeight:600}}>No attendance records yet</p></div>
        : fLogs.map(l=>{
    ...
    {staffSubTab==="roster"&&<>
      ...
      {(staffList||[]).length===0
        ? <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,padding:"36px 20px",textAlign:"center"}}><p style={{color:t.sub,fontSize:14}}>No staff members added yet</p></div>
        : (staffList||[]).map(m=>{
            ...
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                  ...
                  {todayLog
                    ? <span style={{background:statusColor+"20",color:statusColor,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700}}>{todayLog.status} today</span>
                    : <span style={{background:t.inp,color:t.sub,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:600}}>Not logged today</span>
                  }
                </div>
                <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:2}}>
    ...
    {staffSubTab==="management"&&isAdmin&&(()=>{
      // Per-staff summary: total days, present, absent, late, avg hours
      const members = staffList||[];
      const logs = staffLogs||[];
      const summary = members.map(m=>{
        const mLogs = logs.filter(l=>l.staffName===m.name);
        const present = mLogs.filter(l=>l.status==="Present"||l.status==="Late"||l.status==="Half Day").length;
        const absent  = mLogs.filter(l=>l.status==="Absent").length;
        const late    = mLogs.filter(l=>l.status==="Late").length;
        const onLeave = mLogs.filter(l=>l.status==="On Leave").length;
        const hoursArr = mLogs.map(l=>{
          if(!l.inTime||!l.outTime) return null;
          const [ih,im]=(l.inTime||"").split(":").map(Number);
          const [oh,om]=(l.outTime||"").split(":").map(Number);
          if([ih,im,oh,om].some(isNaN)) return null;
          return ((oh*60+om)-(ih*60+im)-(+l.breakMins||0))/60;
        }).filter(x=>x!==null&&x>0);
        const avgHrs = hoursArr.length ? (hoursArr.reduce((a,b)=>a+b,0)/hoursArr.length) : null;
        const attPct = mLogs.length ? Math.round((present/mLogs.length)*100) : null;
        return {...m,mLogs,present,absent,late,onLeave,avgHrs,attPct};
      });
      return <>
        <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <p style={{color:t.text,fontWeight:700,fontSize:14}}>👥 Staff Performance Overview</p>
            <span style={{color:t.sub,fontSize:11}}>{summary.length} members</span>
          </div>
          {summary.length===0
            ? <div style={{padding:"32px",textAlign:"center"}}><p style={{color:t.sub,fontSize:13}}>No staff members in roster yet</p></div>
            : summary.map(m=>(
              <div key={m.id} style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                <div style={{width:36,height:36,borderRadius:10,background:"#2563eb20",border:"1.5px solid #2563eb40",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,fontWeight:800,color:"#2563eb",flexShrink:0}}>
                  {(m.name||"?")[0].toUpperCase()}
                </div>
                <div style={{flex:1,minWidth:140}}>
                  <p style={{color:t.text,fontWeight:700,fontSize:13}}>{m.name}</p>
                  <p style={{color:t.sub,fontSize:11}}>{m.role||"—"}{m.department?` · ${m.department}`:""}</p>
                </div>
                <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                  <div style={{textAlign:"center"}}>
                    <p style={{color:"#10b981",fontWeight:800,fontSize:15}}>{m.present}</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Present</p>
                  </div>
                  <div style={{textAlign:"center"}}>
                    <p style={{color:"#ef4444",fontWeight:800,fontSize:15}}>{m.absent}</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Absent</p>
                  </div>
                  <div style={{textAlign:"center"}}>
                    <p style={{color:"#f59e0b",fontWeight:800,fontSize:15}}>{m.late}</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Late</p>
                  </div>
                  <div style={{textAlign:"center"}}>
                    <p style={{color:"#8b5cf6",fontWeight:800,fontSize:15}}>{m.onLeave}</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Leave</p>
                  </div>
                  {m.avgHrs!==null&&<div style={{textAlign:"center"}}>
                    <p style={{color:"#3b82f6",fontWeight:800,fontSize:15}}>{m.avgHrs.toFixed(1)}h</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Avg/day</p>
                  </div>}
                  {m.attPct!==null&&<div style={{textAlign:"center",minWidth:44}}>
                    <p style={{color:m.attPct>=80?"#10b981":m.attPct>=60?"#f59e0b":"#ef4444",fontWeight:800,fontSize:15}}>{m.attPct}%</p>
                    <p style={{color:t.sub,fontSize:10,marginTop:1}}>Attendance</p>
                  </div>}
                </div>
              </div>
            ))
    ...
    {/* ── REPORTS — admin only ── */}
    {staffSubTab==="reports"&&isAdmin&&(()=>{
      const logs = staffLogs||[];
      // Daily counts for last 7 days
      const tStr2 = today();
      const last7 = Array.from({length:7},(_,i)=>{const d=new Date(tStr2);d.setDate(d.getDate()-i);return d.toISOString().slice(0,10);}).reverse();
      const byDate = last7.map(date=>{
        const dayLogs = logs.filter(l=>l.date===date);
        return {
          date,
          label: new Date(date).toLocaleDateString("en-IN",{weekday:"short",day:"numeric",month:"short"}),
          present: dayLogs.filter(l=>l.status==="Present"||l.status==="Late"||l.status==="Half Day").length,
      ...
      const deptMap={};
      (staffList||[]).forEach(m=>{const d=m.department||"Unassigned";deptMap[d]=(deptMap[d]||0)+1;});
      const depts=Object.entries(deptMap).sort(([,a],[,b])=>b-a);
      // Most frequent absentees
      const absentCount={};
      logs.filter(l=>l.status==="Absent").forEach(l=>{absentCount[l.staffName]=(absentCount[l.staffName]||0)+1;});
      const topAbsent=Object.entries(absentCount).sort(([,a],[,b])=>b-a).slice(0,5);
      return <>
        {/* 7-day trend */}
        <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`}}>
            <p style={{color:t.text,fontWeight:700,fontSize:14}}>📅 Last 7 Days — Daily Attendance</p>
          </div>
            ...
            {byDate.map(d=>{
              ...
              return <div key={d.date}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{color:t.sub,fontSize:11,fontWeight:600}}>{d.label}</span>
                  <span style={{color:t.text,fontSize:11,fontWeight:700}}>{d.present} present · {d.absent} absent{d.late>0?` · ${d.late} late`:""}</span>
                </div>
                <div style={{background:t.inp,borderRadius:99,height:7,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${pct}%`,background:pct>=80?"#10b981":pct>=60?"#f59e0b":"#ef4444",borderRadius:99,transition:"width 0.3s"}}/>
                </div>
              </div>;
            })}
          </div>
        ...
        {/* Department breakdown */}
        {depts.length>0&&<div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`}}>
            <p style={{color:t.text,fontWeight:700,fontSize:14}}>🏢 Staff by Department</p>
          </div>
          <div style={{padding:"12px 18px",display:"flex",flexDirection:"column",gap:8}}>
            {depts.map(([dept,cnt])=>(
              <div key={dept} style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{color:t.text,fontSize:13,fontWeight:600}}>{dept}</span>
                <span style={{background:"#2563eb15",color:"#2563eb",borderRadius:99,padding:"2px 10px",fontSize:12,fontWeight:700}}>{cnt} staff</span>
              </div>
            ))}
          </div>
        </div>}
        {/* Top absentees */}
        {topAbsent.length>0&&<div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`}}>
            <p style={{color:t.text,fontWeight:700,fontSize:14}}>⚠️ Most Absences</p>
          </div>
          <div style={{padding:"12px 18px",display:"flex",flexDirection:"column",gap:8}}>
            {topAbsent.map(([name,cnt])=>(
              <div key={name} style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span style={{color:t.text,fontSize:13,fontWeight:600}}>{name}</span>
                <span style={{background:"#ef444415",color:"#ef4444",borderRadius:99,padding:"2px 10px",fontSize:12,fontWeight:700}}>{cnt} absent day{cnt!==1?"s":""}</span>
              </div>
            ))}
          </div>
        </div>}
    ...
    {/* ── SETTINGS — admin only ── */}
    {staffSubTab==="settings"&&isAdmin&&(()=>{
      const s = settings||{};
      const rows = [
        ["Staff Attendance Feature",s.featureStaffAttendance?"✅ Enabled":"❌ Disabled"],
        ["Default Shift",s.staffDefaultShift||(s.shifts||["Morning"])[0]||"Morning"],
        ["Overtime Threshold",`${s.staffOvertimeThresholdHrs||9} hrs/day`],
        ["Require In/Out Time",s.staffRequireInOutTime?"Yes":"No"],
        ["Show Department",s.staffShowDepartment!==false?"Yes":"No"],
        ["Show Task Field",s.staffShowTask?"Yes":"No"],
        ["Show Break Duration",s.staffShowBreakDuration?"Yes":"No"],
        ["Show Overtime Reason",s.staffShowOvertimeReason?"Yes":"No"],
        ["Allow Custom Name",s.staffAllowCustomName!==false?"Yes":"No"],
        ["Show Salary Type",s.staffShowSalaryType?"Yes":"No"],
        ["Salary Types",(s.staffSalaryTypes||["Monthly","Daily","Per Hour"]).join(", ")],
        ["Attendance Statuses",(s.staffStatuses||["Present","Absent","Half Day","Late","On Leave"]).join(", ")],
        ["Departments",(s.staffDepartments||["Production","Delivery","Packaging","Cleaning","Admin","Other"]).join(", ")],
        ["Staff Roles",(s.staffRoles||["Roti Maker","Packer","Delivery","Cleaner","Supervisor","Admin"]).join(", ")],
        ["Employment Types",(s.staffEmploymentTypes||["Full-time","Part-time","Contract","Daily Wage"]).join(", ")],
      ];
      return <>
        <div style={{background:"#f59e0b10",border:"1px solid #f59e0b30",borderRadius:12,padding:"10px 14px",display:"flex",gap:8,alignItems:"flex-start"}}>
          <span style={{fontSize:16,flexShrink:0}}>⚙️</span>
          <p style={{color:"#f59e0b",fontSize:12,fontWeight:600}}>These are read-only staff module settings. To change them, go to the main <strong>Settings → Staff</strong> section.</p>
        </div>
        <div style={{background:t.card,border:`1px solid ${t.border}`,borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:`1px solid ${t.border}`}}>
            <p style={{color:t.text,fontWeight:700,fontSize:14}}>🔧 Staff Module Configuration</p>
          </div>
          {rows.map(([label,val])=>(
            <div key={label} style={{padding:"11px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <span style={{color:t.sub,fontSize:12,fontWeight:600,flex:1}}>{label}</span>
              <span style={{color:t.text,fontSize:12,fontWeight:700,textAlign:"right",maxWidth:"55%",wordBreak:"break-word"}}>{val}</span>
            </div>
          ))}
        </div>
      </>;
    ...
    <Sheet dm={dm} open={!!staffSh} onClose={()=>setStaffSh(null)} title={staffSh==="add"?"🧑‍🍳 Log Attendance":"✏️ Edit Attendance"}>
      ...
      {staffF.inTime&&staffF.outTime&&(()=>{
        const _ip=staffF.inTime.split(":"),_op=staffF.outTime.split(":");
        if(_ip.length<2||_op.length<2) return null;
        const [ih,im]=_ip.map(Number);
        const [oh,om]=_op.map(Number);
        if(isNaN(ih)||isNaN(im)||isNaN(oh)||isNaN(om)) return null;
